...
const ScrapyLinkedInScraper: React.FC = () => {
    ...
    const fetchJobs = async () => {
        try {
            const response = await fetch('/api/scrapy/api/jobs/?platform=linkedin');
            if (response.ok) {
    ...
    const quickScrape = async () => {
        if (!quickScrapeUrl.trim()) {
            setSnackbar({ open: true, message: 'Please enter a valid LinkedIn URL', severity: 'error' });
            return;
        }
        ...
        try {
            ...
            const createResponse = await fetch('/api/scrapy/api/jobs/create_job/', {
                ...
                body: JSON.stringify({
                    name: `Quick LinkedIn Scrape - ${new Date().toLocaleString()}`,
                    project_id: parseInt(projectId),
                    platform: 'linkedin',
                    content_type: 'posts',
    ...
    return (
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
            ...
            <Alert severity="warning" sx={{ mb: 3 }}>
                <strong>LinkedIn Anti-Bot Notice:</strong> LinkedIn has strict anti-scraping measures. 
                This scraper works best with public company pages and may require login for some content. 
            ...
            <Card sx={{ mb: 3 }}>
                <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        Enter a LinkedIn company page or profile URL to quickly scrape posts and see results automatically.
                    </Typography>
                    
                    <Stack direction="row" spacing={2} alignItems="flex-start">
                        <TextField
                            label="LinkedIn URL"
                            placeholder="https://www.linkedin.com/company/company-name/ or https://www.linkedin.com/in/profile-name/"
                            value={quickScrapeUrl}
                            onChange={(e) => setQuickScrapeUrl(e.target.value)}
                            fullWidth
                            variant="outlined"
                            helperText="Examples: https://www.linkedin.com/company/linkedin/ or https://www.linkedin.com/in/username/"
                        />
                        <Button
                            variant="contained"
                            onClick={quickScrape}
                                        ...
                                        {result.success && result.scraped_data && Array.isArray(result.scraped_data) && (
                                            <Box>
                                                <Typography variant="subtitle2" sx={{ mb: 1 }}>
                                                    Items Found: {result.scraped_data.length}
                                                </Typography>
                                                {result.scraped_data.slice(0, 3).map((item: any, itemIndex: number) => (
                                                    <Paper key={itemIndex} variant="outlined" sx={{ p: 2, mb: 1 }}>
                                                        {item.content_type === 'profile' ? (
                                                            ...
                                                            <Box>
                                                                {item.text && (
                                                                    <Typography variant="body2" sx={{ mb: 1 }}>
                                                                        <strong>Post:</strong> {item.text.substring(0, 150)}
                                                ...
                                                {result.scraped_data.length > 3 && (
                                                    <Typography variant="caption" color="text.secondary">
                                                        ... and {result.scraped_data.length - 3} more items
                                                    </Typography>
                                                )}
                                            </Box>
                                        )}
                            ...
                            <TableBody>
                                {jobs.map((job) => (
                                    <TableRow key={job.id}>
                                        ...
                                        <TableCell>{job.target_urls?.[0]?.includes('/company/') ? 'Company' : job.target_urls?.[0]?.includes('/in/') ? 'Profile' : 'Posts'}</TableCell>
                                        <TableCell>
                                            <Box>
                                                <Typography variant="body2">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={8} align="center">
                                            <Typography variant="body2" color="text.secondary">
                                                No LinkedIn scraping jobs yet. Create your first job to get started!
                                            </Typography>
                                        </TableCell>
                                    </TableRow>
                                )}
            ...
            <Dialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} maxWidth="md" fullWidth>
                <DialogTitle>Create LinkedIn Scraping Job</DialogTitle>
                <DialogContent>
                    <Stack spacing={3} sx={{ mt: 2 }}>
                        <TextField
                            ...
                            fullWidth
                            placeholder="e.g., Company Posts Analysis"
                        />
                        ...
                        <FormControl fullWidth>
                            ...
                            <FormHelperText>
                                Posts: Scrape posts from company pages or profiles | Profile: Get profile information
                            </FormHelperText>
                        </FormControl>
                        ...
                        <TextField
                            ...
                            helperText="Maximum number of posts to scrape per URL (1-50)"
                        />
                        ...
                        {formData.target_urls.map((url, index) => (
                            <Stack key={index} direction="row" spacing={2} alignItems="flex-start">
                                <TextField
                                    label={`LinkedIn URL ${index + 1}`}
                                    value={url}
                                    onChange={(e) => updateUrl(index, e.target.value)}
                                    placeholder="https://www.linkedin.com/company/company-name/"
                                    fullWidth
                                    helperText={index === 0 ? "Company pages or profile URLs work best" : ""}
                                />
                                <TextField
                                    label="Source Name"
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    if (jobResults.length > 0) {
                                        // Extract LinkedIn posts from results to match CSV format
                                        const posts = jobResults.map(result => result.scraped_data).filter(Boolean);
                                        const dataStr = JSON.stringify(posts, null, 2);
                                        const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
                                        const exportFileDefaultName = `${selectedJob?.name?.replace(/\s+/g, '_')}_linkedin_posts.json`;
                ...
                <DialogContent>
                    <Box sx={{ mt: 1 }}>
                        {jobResults.length === 0 ? (
                            ...
                            <>
                                {/* LinkedIn Sentiment Analysis Dashboard */}
                                {(() => {
                                    // Extract LinkedIn posts from results
                                    const posts = jobResults
                                    ...
                                    // Simple sentiment analysis for LinkedIn content
                                    ...
                                    const sentimentData = posts.map(post => {
                                        const textContent = post.text || post.description || post.post_text || post.content || '';
                                        const postSentiment = analyzeSentiment(textContent);
                                        return {
                                            ...post,
                                            overallSentiment: postSentiment,
                                            textContent
                                        };
                                    });
                                    ...
                                    const totalEngagement = posts.reduce((sum, post) => sum + (post.likes || post.num_likes || 0) + (post.comments || post.num_comments || 0) + (post.shares || post.num_shares || 0), 0);
                                    const avgLikes = Math.round(posts.reduce((sum, post) => sum + (post.likes || post.num_likes || 0), 0) / posts.length);
                                    const totalComments = posts.reduce((sum, post) => sum + (post.comments || post.num_comments || 0), 0);
                                    const totalShares = posts.reduce((sum, post) => sum + (post.shares || post.num_shares || 0), 0);
                                                ...
                                                <CardContent sx={{ p: 2 }}>
                                                    ...
                                                    <TableContainer sx={{ maxHeight: 400, overflowY: 'auto' }}>
                                                        <Table size="small" stickyHeader>
                                                            ...
                                                            <TableBody>
                                                                {sentimentData
                                                                    .sort((a, b) => (b.likes || b.num_likes || 0) - (a.likes || a.num_likes || 0))
                                                                    .slice(0, 10)
                                                                    .map((post, index) => (
                                                                    <TableRow key={index} hover>
                                                                        <TableCell>
                                                                            ...
                                                                            >
                                                                                <LinkedInIcon sx={{ fontSize: 20, color: '#0077b5', mr: 1 }} />
                                                                                <Box>
                                                                        ...
                                                                        <TableCell>
                                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                                                ...
                                                                                <Typography variant="body2" color="black">{(post.comments || post.num_comments || 0)}</Typography>
                                                                            </Box>
                                                                        </TableCell>
                                                                        <TableCell>
                                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                                        ...
                                                                        <TableCell>
                                                                            <Typography variant="body2" color="black" fontWeight="bold">
                                                                                {(((post.likes || post.num_likes) || 0) + ((post.comments || post.num_comments) || 0) + ((post.shares || post.num_shares) || 0)).toLocaleString()}
                                                                            </Typography>
                                                                        </TableCell>
                                                                    </TableRow>
                                                                ))}
...
export default ScrapyLinkedInScraper;