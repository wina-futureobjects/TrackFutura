...
const ScrapyInstagramScraper: React.FC = () => {
    ...
    const [enhancedSentimentData, setEnhancedSentimentData] = useState<any>(null);
    ...
    const performEnhancedSentimentAnalysis = async (jobId: number) => {
        setSentimentAnalysisLoading(true);
        try {
            const response = await fetch('/api/scrapy/api/enhanced-sentiment-analysis/', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                setEnhancedSentimentData(sentimentData);
            } else {
                ...
                console.error('Enhanced sentiment analysis failed:', errorMessage);
                setSnackbar({ 
                    open: true, 
                    message: `Enhanced sentiment analysis failed: ${errorMessage}`, 
                    severity: 'warning' 
                });
            }
        } catch (error) {
            console.error('Error performing enhanced sentiment analysis:', error);
            setSnackbar({ 
                open: true, 
                message: 'Failed to perform enhanced sentiment analysis', 
                severity: 'warning' 
            });
        } finally {
    ...
    const quickScrape = async () => {
        if (!quickScrapeUrl.trim()) {
            setSnackbar({ open: true, message: 'Please enter a valid Instagram URL', severity: 'error' });
            return;
        }
        ...
        try {
            ...
            console.log('Creating job with data:', {
                name: `Quick Instagram Scrape - ${new Date().toLocaleString()}`,
                project_id: parseInt(projectId),
                platform: 'instagram',
                content_type: 'posts',
            ...
            // Create Bright Data scrape request
            const createResponse = await apiFetch('brightdata/requests/trigger_instagram_scrape/', {
                method: 'POST',
                body: JSON.stringify({
                    target_url: quickScrapeUrl,
                    content_type: 'post',
                    num_of_posts: quickScrapeOptions.num_of_posts,
                    start_date: quickScrapeOptions.start_date ? quickScrapeOptions.start_date.toISOString().split('T')[0] : '',
                    end_date: quickScrapeOptions.end_date ? quickScrapeOptions.end_date.toISOString().split('T')[0] : '',
                }),
            });
            ...
            if (!createResponse.ok) {
                ...
                throw new Error(`Failed to create scrape request: ${errorText}`);
            }

            const response = await createResponse.json();
            console.log('Bright Data response:', response);

            // Bright Data returns success status and snapshot_id
            if (response.status === 'Scraper request sent successfully') {
                setSnackbar({ 
                    open: true, 
                    message: `Instagram scrape request sent successfully! Snapshot ID: ${response.brightdata_response?.snapshot_id}`, 
                    severity: 'success' 
                });
                
                // Set a mock job object for UI consistency
                const mockJob = {
                    id: response.brightdata_response?.snapshot_id || 'unknown',
                    name: `Quick Instagram Scrape - ${new Date().toLocaleString()}`,
                    status: 'processing',
                    platform: 'instagram',
                    created_at: new Date().toISOString()
                };
                setQuickScrapeJob(mockJob);
            } else {
            ...
            // Bright Data will send results via webhook
            // Set loading to false since the request has been sent
            setQuickScrapeLoading(false);
    ...
    const downloadResults = (results: ScrapyResult[], filename: string = 'instagram-scraping-results') => {
        try {
    ...
    const downloadResultsAsCSV = (results: ScrapyResult[], filename: string = 'instagram-scraping-results') => {
        try {
            // Flatten the data for CSV export
            const csvRows = [];
            const headers = ['Source Name', 'Source URL', 'Success', 'Caption', 'Likes', 'Comments', 'Shares', 'Timestamp', 'Post URL', 'Media Type'];
    ...
    return (
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
            ...
            <Card sx={{ mb: 3 }}>
                <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        Enter an Instagram profile URL to quickly scrape posts and see results automatically.
                    </Typography>
                    ...
                    <Stack direction="row" spacing={2} alignItems="flex-start" sx={{ mb: 2 }}>
                        <TextField
                            ...
                            fullWidth
                            variant="outlined"
                            helperText="Examples: https://www.instagram.com/instagram/ or https://www.instagram.com/username/"
                        />
                        <Button
                            variant="contained"
                            onClick={quickScrape}
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                                ...
                                <Stack direction="row" spacing={1}>
                                    <Button
                                        ...
                                        onClick={() => downloadResults(quickScrapeResults, 'quick-instagram-scrape')}
                                    >
                                        Download JSON
                                    </Button>
                                    <Button
                                        ...
                                        size="small"
                                        startIcon={<DownloadIcon />}
                                        onClick={() => downloadResultsAsCSV(quickScrapeResults, 'quick-instagram-scrape')}
                                    >
                                        Download CSV
                                    </Button>
                                </Stack>
                                                    ...
                                                    <Paper key={postIndex} variant="outlined" sx={{ p: 2, mb: 1 }}>
                                                        {post.caption && (
                                                            <Typography variant="body2" sx={{ mb: 1 }}>
                                                                <strong>Caption:</strong> {post.caption.substring(0, 100)}
                                ...
                                {jobs.length === 0 && (
                                    <TableRow>
                                        <TableCell colSpan={8} align="center">
                                            No jobs found. Create your first Instagram scraping job!
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
            ...
            <Dialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} maxWidth="md" fullWidth>
                <DialogTitle>Create Instagram Scraping Job</DialogTitle>
                <DialogContent>
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    if (jobResults.length > 0) {
                                        ...
                                        const exportFileDefaultName = `${selectedJob?.name?.replace(/\s+/g, '_')}_instagram_posts.json`;
                    ...
                    <Box sx={{ mt: 1 }}>
                        {jobResults.length === 0 ? (
                            ...
                            <>
                                ...
                                {(() => {
                                    // Use enhanced sentiment data if available, otherwise fall back to basic analysis
                                    let sentimentData, sentimentStats, posts;
                                    
                                    if (enhancedSentimentData && enhancedSentimentData.success) {
                                        // Use OpenAI-enhanced sentiment analysis
                                        posts = enhancedSentimentData.posts;
                                        sentimentStats = enhancedSentimentData.sentiment_stats;
                                        
                                        sentimentData = posts.map(post => ({
                                            ...post,
                                            overallSentiment: post.sentiment,
                                            captionSentiment: post.sentiment
                                        }));
                                    } else {
                                        // Fallback to basic analysis
                                        const rawPosts = jobResults.map(result => result.scraped_data).filter(Boolean);
                                        ...
                                        sentimentData = rawPosts.map(post => {
                                            const captionSentiment = analyzeSentiment(post.caption || '');
                                            return {
                                                ...post,
                                                captionSentiment,
                                                overallSentiment: captionSentiment
                                            };
                                        });
                                            ...
                                            <Card sx={{ mb: 4 }}>
                                                <CardContent>
                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3, justifyContent: 'center' }}>
                                                        <AnalyticsIcon />
                                                        <Typography variant="h6">
                                                            {enhancedSentimentData && enhancedSentimentData.success ? 'AI-Enhanced Sentiment Analysis' : 'Sentiment Analysis'}
                                                        </Typography>
                                                        {enhancedSentimentData && enhancedSentimentData.success && (
                                                            <Chip 
                                                                label="AI Powered" 
                                                                color="primary" 
                                                                size="small" 
                                                    ...
                                                    {sentimentAnalysisLoading && (
                                                        <Box sx={{ textAlign: 'center', mb: 3 }}>
                                                            <Typography variant="body2" color="text.secondary">
                                                                Analyzing sentiment using AI... This may take a few moments.
                                                            </Typography>
                                                        </Box>
                                                    )}
                                                                    ...
                                                                    <TableRow key={index} hover>
                                                                        ...
                                                                        <TableCell>
                                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                                                ...
                                                                                <Typography variant="body2" color="black">{post.comments_count || 0}</Typography>
                                                                            </Box>
                                                                        </TableCell>
                                                                        <TableCell>
                                                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                                                        ...
                                                                        <TableCell>
                                                                            <Typography variant="body2" color="black" fontWeight="bold">
                                                                                {((post.likes || 0) + (post.comments_count || 0) + (post.shares || 0)).toLocaleString()}
                                                                            </Typography>
                                                                        </TableCell>
                                                                    </TableRow>
                                                                ))}
...
export default ScrapyInstagramScraper;