...
class LinkedInPostViewSet(viewsets.ModelViewSet):
    ...
    def scrape_from_urls(self, request):
        """
        Scrape LinkedIn posts from URLs using Apify
        """
        try:
            import asyncio
            ...
            folder_name = request.data.get('folder_name', 'LinkedIn Scraped Data')
            ...
            source_names = [f"LinkedIn Source {i+1}" for i in range(len(urls))]
            ...
            async def create_and_start_job():
                ...
                job = await service.create_scraping_job(
                    name=f"LinkedIn URL Scrape - {folder_name}",
            ...
            if success:
                return Response({
                    'success': True,
                    'message': 'LinkedIn scraping job started successfully',
                    'job_id': job.id,
                    'job_name': job.name,
                    'status': job.status,
                    'urls_count': len(urls)
            ...
            return Response({
                'success': False,
                'error': f'Error starting LinkedIn scrape: {str(e)}'
            }, status=status.HTTP_400_BAD_REQUEST) 